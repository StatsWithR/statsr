...
#' @param population parameter to estimate: mean or proportion
                      ...
                      show_res = verbose)
{
  ...
  if (length(type) > 1) {
    stop("Missing type: ci (credible interval) or ht (hypothesis test)", call. = FALSE)
  }
  if (length(method) > 1) {
  ...
  if(type == "ht"){
    ...
    if (alternative != "twosided")
      stop("Only two sided tests are currently supported.", call. = FALSE)
  }
